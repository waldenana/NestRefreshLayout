...
public abstract class AbsListLoader extends ViewGroup {
    ...
    //region field
    protected static final String TAG = AbsListLoader.class.getSimpleName();
     ...
     */
    public AbsListLoader(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context, null, R.attr.pageListLoaderStyle);
    }
     ...
     * @param attrs
     */
    public AbsListLoader(Context context, AttributeSet attrs) {
    ...
    public AbsListLoader(View listView) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.AbsPageListLoader, defStyleAttr, 0);
        final int N = a.getIndexCount();
        int resFooter = R.layout.layout_loadmore;
        for (int i = 0; i < N; i++) {
            int attr = a.getIndex(i);
            if (attr == R.styleable.AbsPageListLoader_loadMoreLayout) {
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (t > 0 && !isLoadingMore() && isLoadMoreEnable() && !isLoadAll()) {
            if (t > mFooterView.getMeasuredHeight()) {
                ((PageListLoader.LoaderDecor) mFooterView).setState(PullFooter.STATE_READY);
            } else {
                ((PageListLoader.LoaderDecor) mFooterView).setState(PullFooter.STATE_NORMAL);
            }
        }
    }
    ...
    public void onLoadAll() {
        ...
        mbLoadAll = true;
        if (mFooterView != null)
            ((PageListLoader.LoaderDecor) mFooterView).setState(PullHeader.STATE_ALL);
        resetView();
    }
